<template>
  <div class="app-container">
    <el-row :gutter="24" class="filter-container">
      <el-col :span="6">
        <el-input v-model="queryParam.name" class="filter-item" placeholder="请输入家庭旅馆名称查询" />
      </el-col>
      <el-col :span="12">
    ...
    >
      ...
      <el-table-column align="center" label="家庭旅馆名称" prop="name" />
    ...
    >
      <div class="drawer__content">
        <el-form ref="entityDataForm" :model="entityData" :rules="rules" label-width="120px">
          <el-form-item label="家庭旅馆名称" prop="name">
...
import { enableFamilyHotel, disableFamilyHotel }
  from '@/api/channel/family-hotel-api'
...
export default {
  ...
  data() {
    return {
      apiBaseUrl: '/channel/familyhotels',
      ...
      rules: {
        name: [
          { required: true, message: '请输入家庭旅馆名称', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    handleStatusConfirmChange(index, row) {
      if (row.status === 1) {
        ...
        disableFamilyHotel(row.id).then(() => {
          this.$notify.success({
            title: '成功',
            message: '禁用成功'
          })
